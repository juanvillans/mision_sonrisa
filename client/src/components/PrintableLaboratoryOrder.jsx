...
const PrintableContent = forwardRef((props, ref) => {
  ...
  return (
    <div ref={ref} className="w-full mx-auto bg-white relative">
      ...
      >
        ...
        <div className="text-center font-bold text-xs gap-3 my-3  mt-2 text-color1">
      ...
      <div className="max-w-md mx-auto overflow-hidden rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
  ...
  <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 pb-4">
    <div>
      <span className="text-xs font-semibold uppercase tracking-wider text-gray-400 block">Nro de Caso</span>
      <span className="text-xl font-bold tracking-tight text-blue-600">
        #{props.data?.id || '—'}
      </span>
    </div>
    <div className="text-right">
      <span className="text-xs font-semibold uppercase tracking-wider text-gray-400 block">Fecha</span>
  ...
  <dl className="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
    ...
    <div className="sm:col-span-2 border-b border-gray-50 pb-2">
      ...
      <dd className="text-base font-semibold text-gray-900">{props.data?.name || '—'}</dd>
    </div>

    {/* Compact grid items */}
    <div>
      <dt className="text-xs font-medium text-gray-500">C.I.</dt>
      <dd className="text-sm font-medium text-gray-900">{props.data?.ci || '—'}</dd>
    </div>

    <div>
      <dt className="text-xs font-medium text-gray-500">Teléfono</dt>
      <dd className="text-sm font-medium text-gray-900">{props.data?.phone || '—'}</dd>
    </div>

    <div>
      <dt className="text-xs font-medium text-gray-500">Edad / Sexo</dt>
      <dd className="text-sm font-medium text-gray-900">
        {props.data?.age ? `${props.data.age} años` : '—'} • {props.data?.sex || '—'}
      </dd>
    </div>

    <div>
      <dt className="text-xs font-medium text-gray-500">Color de dientes</dt>
      <dd className="text-sm font-medium text-gray-900">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-800">
          <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: TOOTH_COLOR_OPTIONS.find((c) => c.value === props.data?.tooth_color)?.color }} />
    ...
    <div className="sm:col-span-2 pt-1">
      <dt className="text-xs font-medium text-gray-500">Tipo de Prótesis</dt>
